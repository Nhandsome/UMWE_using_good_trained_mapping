...
## Nhand : path for save good predictions
import pickle 
SAVE_EVAL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'debug_file', 'prediction')
...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval):
    ...
    elif dico_eval == '5000-10000':
        path = os.path.join(DIC_EVAL_PATH, '%s-%s.5000-10000.txt' % (lang1, lang2))
    elif dico_eval == 'all':
...
def get_word_translation_accuracy_n(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval):
    """
    Given source and target word embeddings, and a dictionary,
    evaluate the translation accuracy using the precision@k.
    """
...
## Nhand
        ...
        for i, src_id in enumerate(dico[:, 0].cpu().numpy()):
            matching[src_id] = min(matching.get(src_id, 0) + _matching[i], 1)
            if (k==1) & (len(dico)>0):
                mat_dic[src_id] = [dico[i,1].numpy().tolist(), matching[src_id]]
## Nhand
        if k == 1 :
            path = os.path.join(SAVE_EVAL_PATH, '%s-%s.txt' % (lang1, lang2))
            with open(path, "wb") as fw:
                pickle.dump(mat_dic, fw)
