...
from .dico_builder import build_dictionary, build_dictionary_multi, build_dictionary_multi_2, build_dictionary_multi_3
...
# HAN
    def load_good_mapping(self):
        """
        Load good mappings from good_map_path.
        """
        for src_lang, tgt_lang in itertools.permutations(self.params.src_langs,2):
            path = os.path.join(self.params.good_map_path,
                                f'best_mapping_{src_lang}2{tgt_lang}.t7')
            logger.info(f'* Reloading trained good mapping {src_lang} to {tgt_lang} model from {path} ...')
            assert os.path.isfile(path)
            to_reload = torch.from_numpy(torch.load(path))
            g_map_name = f'{src_lang}_{tgt_lang}'
            W = self.mappings[g_map_name].weight.detach()
            assert to_reload.size() == W.size()
            W.copy_(to_reload.type_as(W))

# HAN
    def new_build_dictionary(self):
        """
        Build dictionaries from aligned embeddings.
        It will use original aligned embeddings and also embeddings using trained good mapping.
        """
        # build dicos for all lang pairs
        logger.info(f'new_build_dictionary function')
        for i, lang1 in enumerate(self.params.all_langs):
            for j, lang2 in enumerate(self.params.all_langs):
                if i != j:
                    pivot_lang_list = self.params.all_langs.copy()
                    pivot_lang_list.remove(lang1)
                    pivot_lang_list.remove(lang2)

                    if lang2 == self.params.tgt_lang :
                        logger.info(f'dictionary : {lang1}_{lang2}_new')
                        # create pivot mapping using trained good mapping
                        # FOR JUST ONE PIVOT MAPPING NOW
                        for pivot_lang in pivot_lang_list:
                            lang_p = f'{lang1}_{pivot_lang}'
                            # src -> pivot -> tgt 
                            src_emb = self.embs[lang1].weight
                            src_emb_p = apply_mapping(self.mappings[lang_p], src_emb).detach()
                            src_emb_p = apply_mapping(self.mappings[pivot_lang], src_emb_p).detach()
                            # srt -> tgt
                            src_emb_d = apply_mapping(self.mappings[lang1], src_emb).detach()
                            # tgt = tgt
                            tgt_emb = self.embs[lang2].weight
                            tgt_emb = apply_mapping(self.mappings[lang2], tgt_emb).detach()

                            src_emb_d = src_emb_d / src_emb_d.norm(2, 1, keepdim=True).expand_as(src_emb_d)
                            src_emb_p = src_emb_p / src_emb_p.norm(2, 1, keepdim=True).expand_as(src_emb_p)
                            tgt_emb = tgt_emb / tgt_emb.norm(2, 1, keepdim=True).expand_as(tgt_emb)

                            self.dicos[(lang1, lang2)] = build_dictionary_multi(src_emb_p, src_emb_d, tgt_emb, self.params)

                    elif self.params.tgt_lang not in [lang1, lang2]:
                        if i > j :
                            logger.info(f'dictionary : {lang1}_{lang2}_new')
                            self.dicos[(lang1, lang2)] = self.dicos[(lang2, lang1)][:, [1,0]]
                            continue
                        ...
                        logger.info(f'dictionary : {lang1}_{lang2}_new')
                        logger.info(f'build dictionary using trained good mapping')
                        # create direct mapping using trained good mapping
                        # JUST MAPPING TO LANG2 NOW
                        good_map_1 = f'{lang1}_{lang2}'
                        # lang1 -> lang2 (GOOD mapping)
                        good_emb_1 = self.embs[lang1].weight
                        good_emb_1 = apply_mapping(self.mappings[good_map_1], good_emb_1).detach()
                        # lang2 = lang2 (GOOD target)
                        tgt_emb_1 = self.embs[lang2].weight
                        tgt_emb_1 = apply_mapping(self.mappings[self.params.tgt_lang], tgt_emb_1).detach()

                        good_map_2 = f'{lang2}_{lang1}'
                        # lang1 -> lang2 (GOOD mapping)
                        good_emb_2 = self.embs[lang1].weight
                        good_emb_2 = apply_mapping(self.mappings[good_map_2], good_emb_2).detach()
                        # lang2 = lang2 (GOOD target)
                        tgt_emb_2 = self.embs[lang1].weight
                        tgt_emb_2 = apply_mapping(self.mappings[self.params.tgt_lang], tgt_emb_2).detach()

                        tmp_dico_1 = build_dictionary_multi_3(good_emb_1, tgt_emb_1, good_emb_2, tgt_emb_2, self.params)

                        logger.info(f'build dictionary using origin pivot mapping')
                        src_emb = self.embs[lang1].weight
                        src_emb = apply_mapping(self.mappings[lang1], src_emb).detach()
                        tgt_emb = self.embs[lang2].weight
                        tgt_emb = apply_mapping(self.mappings[lang2], tgt_emb).detach()
                        src_emb = src_emb / src_emb.norm(2, 1, keepdim=True).expand_as(src_emb)
                        tgt_emb = tgt_emb / tgt_emb.norm(2, 1, keepdim=True).expand_as(tgt_emb)
                        tmp_dico_2 = build_dictionary(src_emb, tgt_emb, self.params)

                        tmp_dico_good = set([(a, b) for a, b in tmp_dico_1.cpu().numpy()])
                        tmp_dico_origin = set([(a, b) for a, b in tmp_dico_2.cpu().numpy()])
                        dico_total = tmp_dico_good | tmp_dico_origin

                        logger.info(f'New union dictionary of {len(dico_total)} pairs')
                        dico = torch.LongTensor(list([[int(a), int(b)] for (a, b) in dico_total]))

                        self.dicos[(lang1, lang2)] = dico.to(self.params.device)
                        

                        # # lang1 -> lang2 -> tgt / lang1 -> tgt / lang2 -> lang1 -> tgt / lang2 -> tgt
                        # lang_p1 = f'{lang1}_{lang2}'
                        # lang_p2 = f'{lang2}_{lang1}'
                        # # src -> pivot -> tgt 
                        # src_emb_1 = self.embs[lang1].weight
                        # src_emb_p1 = apply_mapping(self.mappings[lang_p1], src_emb_1).detach()
                        # src_emb_p1 = apply_mapping(self.mappings[lang2], src_emb_p1).detach()

                        # src_emb_2 = self.embs[lang2].weight
                        # src_emb_p2 = apply_mapping(self.mappings[lang_p2], src_emb_2).detach()
                        # src_emb_p2 = apply_mapping(self.mappings[lang1], src_emb_p2).detach()

                        # # srt -> tgt
                        # src_emb_d1 = apply_mapping(self.mappings[lang1], src_emb_1).detach()
                        # src_emb_d2 = apply_mapping(self.mappings[lang2], src_emb_2).detach()

                        # src_emb_p1 = src_emb_p1 / src_emb_p1.norm(2, 1, keepdim=True).expand_as(src_emb_p1)
                        # src_emb_p2 = src_emb_p2 / src_emb_p2.norm(2, 1, keepdim=True).expand_as(src_emb_p2)
                        # src_emb_d1 = src_emb_d1 / src_emb_d1.norm(2, 1, keepdim=True).expand_as(src_emb_d1)
                        # src_emb_d2 = src_emb_d2 / src_emb_d2.norm(2, 1, keepdim=True).expand_as(src_emb_d2)

                        # tmp_dico_2 = build_dictionary_multi_2(src_emb_p1, src_emb_p2, src_emb_d1, src_emb_d2, self.params)

                        # tmp_dico_3 = torch.cat([tmp_dico_1, tmp_dico_2], dim=0)
                    ...
                    logger.info(f'dictionary : {lang1}_{lang2}')
                    src_emb = self.embs[lang1].weight
                    src_emb = apply_mapping(self.mappings[lang1], src_emb).detach()
                    tgt_emb = self.embs[lang2].weight
                    tgt_emb = apply_mapping(self.mappings[lang2], tgt_emb).detach()
    ...
    def mpsr_step(self, stats):
        # loss
        loss = 0
        for lang1 in self.params.src_langs:
            lang2 = random.choice(self.params.all_langs)
...
# HAN
    def save_best_before_mpsr(self, to_log, metric):
        """
        Save the best unsupervised model before doing mpsr tuning for the given validation metric.
        """
        # best mapping for the given validation criterion
        if to_log[metric] == self.best_valid_metric:
            # new best mapping
            ...
            for src_lang in self.params.src_langs:
                W = self.mappings[src_lang].weight.detach().cpu().numpy()
                path = os.path.join(self.params.exp_path,
                                    f'best_mapping_before_mpsr_{src_lang}2{tgt_lang}.t7')
                logger.info(f'* Saving the {src_lang} to {tgt_lang} mapping to %s ...' % path)
                torch.save(W, path)
...
# HAN
# Reload pre-trained MAT mapping
    def reload_best_pre_train(self):
        """
        Reload pre-trained MAT mapping
        """
        tgt_lang = self.params.tgt_lang
        for src_lang in self.params.src_langs:
            path = os.path.join(self.params.pre_mat_path,
                                f'best_mapping_before_mpsr_{src_lang}2{tgt_lang}.t7')
            logger.info(f'* Reloading the best pre trained mapping {src_lang} to {tgt_lang} model from {path} ...')
            # reload the model
            assert os.path.isfile(path)
            to_reload = torch.from_numpy(torch.load(path))
            W = self.mappings[src_lang].weight.detach()
