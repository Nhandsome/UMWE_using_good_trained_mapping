...
# HAN
def build_dictionary_multi(src_emb_p, src_emb_d, tgt_emb, params, s2t_candidates=None, t2s_candidates=None):
    """
    Build a training dictionary given current embeddings / mapping.
    Use 2 embeddings one is from original direct mapping and aonter is from pivot mapping using trained good mapping.
    """
    logger.info("Building the train dictionary ...")
    s2t = 'S2T' in params.dico_build
    t2s = 'T2S' in params.dico_build
    ...
    if s2t:
        if s2t_candidates is None:
            s2t_candidates_d = get_candidates(src_emb_d, tgt_emb, params)
            s2t_candidates_p = get_candidates(src_emb_p, tgt_emb, params)
    if t2s:
        if t2s_candidates is None:
            t2s_candidates_d = get_candidates(tgt_emb, src_emb_d, params)
            t2s_candidates_p = get_candidates(tgt_emb, src_emb_p, params)
        t2s_candidates_d = torch.cat([t2s_candidates_d[:, 1:], t2s_candidates_d[:, :1]], 1)
        t2s_candidates_p = torch.cat([t2s_candidates_p[:, 1:], t2s_candidates_p[:, :1]], 1)

    s2t_candidates_d = set([(a, b) for a, b in s2t_candidates_d.numpy()])
    s2t_candidates_p = set([(a, b) for a, b in s2t_candidates_p.numpy()])
    s2t_candidates_t = s2t_candidates_d | s2t_candidates_p
    s2t_candidates = torch.LongTensor(list([[int(a), int(b)] for (a, b) in s2t_candidates_t]))

    t2s_candidates_d = set([(a, b) for a, b in t2s_candidates_d.numpy()])
    t2s_candidates_p = set([(a, b) for a, b in t2s_candidates_p.numpy()])
    t2s_candidates_t = t2s_candidates_d | t2s_candidates_p
    t2s_candidates = torch.LongTensor(list([[int(a), int(b)] for (a, b) in t2s_candidates_t]))
...
# HAN
def build_dictionary_multi_2(src_emb_p1, src_emb_p2, src_emb_d1, src_emb_d2, params, s2t_candidates=None, t2s_candidates=None):
    """
    Build a training dictionary given current embeddings / mapping.
    Use 2 pivot embeddings and 2 direct(original) embeddings
    """
    logger.info("Building the train dictionary ...")
    s2t = 'S2T' in params.dico_build
    t2s = 'T2S' in params.dico_build
    ...
    if s2t:
        if s2t_candidates is None:
            s2t_candidates_d = get_candidates(src_emb_d1, src_emb_d2, params)
            s2t_candidates_p = get_candidates(src_emb_p1, src_emb_d2, params)
    if t2s:
        if t2s_candidates is None:
            t2s_candidates_d = get_candidates(src_emb_d2, src_emb_d1, params)
            t2s_candidates_p = get_candidates(src_emb_p2, src_emb_d1, params)
        t2s_candidates_d = torch.cat([t2s_candidates_d[:, 1:], t2s_candidates_d[:, :1]], 1)
        t2s_candidates_p = torch.cat([t2s_candidates_p[:, 1:], t2s_candidates_p[:, :1]], 1)
...
# HAN
def build_dictionary_multi_3(good_emb_1, tgt_emb_1, good_emb_2, tgt_emb_2, params, s2t_candidates=None, t2s_candidates=None):
    """
    Build a training dictionary given current embeddings / mapping.
    Use 2 pivot embeddings and 2 direct(original) embeddings
    """
    ...
    if s2t:
        if s2t_candidates is None:
            s2t_candidates_1 = get_candidates(good_emb_1, tgt_emb_1, params)    
            s2t_candidates_2 = get_candidates(good_emb_2, tgt_emb_2, params)
    if t2s:
        if t2s_candidates is None:
            t2s_candidates_1 = get_candidates(tgt_emb_1, good_emb_1, params)
            t2s_candidates_2 = get_candidates(tgt_emb_2, good_emb_2, params)
        t2s_candidates_1 = torch.cat([t2s_candidates_1[:, 1:], t2s_candidates_1[:, :1]], 1)
        t2s_candidates_2 = torch.cat([t2s_candidates_2[:, 1:], t2s_candidates_2[:, :1]], 1)

    s2t_candidates_1 = set([(a, b) for a, b in s2t_candidates_1.numpy()])
    s2t_candidates_2 = set([(a, b) for a, b in s2t_candidates_2.numpy()])
    s2t_candidates_t = s2t_candidates_1 | s2t_candidates_2
    s2t_candidates = torch.LongTensor(list([[int(a), int(b)] for (a, b) in s2t_candidates_t]))

    t2s_candidates_1 = set([(a, b) for a, b in t2s_candidates_1.numpy()])
    t2s_candidates_2 = set([(a, b) for a, b in t2s_candidates_2.numpy()])
    t2s_candidates_t = t2s_candidates_1 | t2s_candidates_2
    t2s_candidates = torch.LongTensor(list([[int(a), int(b)] for (a, b) in t2s_candidates_t]))
