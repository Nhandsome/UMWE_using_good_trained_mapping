...
# HAN
parser.add_argument("--good_map_path", type=str, default="", help="Where to load trained good_mappings")
parser.add_argument("--pre_mat_path", type=str, default="", help="Where to load pre-trained best MAT mapping")
parser.add_argument("--use_good_map", type=int, default=0, help="Set to use GOOD mappings (1 to use GOOD mappings)")
...
# HAN
# load trained good mappings from good_map_path
trainer.load_good_mapping()
...
# HAN
logger.info('----> MULTILINGUAL PSEUDO-SUPERVISED REFINEMENT WITH PRE TRAINED MAT MAPPING <----\n\n')
trainer.reload_best_pre_train()
...
for n_epoch in range(params.n_refinement + 1):
    ...
    # HAN
    if params.use_good_map == 0:
        # build a dictionary from refiened mapping
        trainer.build_dictionary()

    else:
        # build a dictionary from original and pivot embeddings
        trainer.new_build_dictionary()        
